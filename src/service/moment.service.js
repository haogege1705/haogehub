...
class MomentService {
  ...
  async getMomentById(momentId) {
    const statement = `SELECT 
      ...
      IF(COUNT(l.id),JSON_ARRAYAGG(
       ...
       (SELECT JSON_ARRAYAGG(CONCAT('http://121.41.115.226:8000/moment/images/', file.filename)) FROM file WHERE m.id = file.moment_id) image
       FROM moment m
  ...
  async getMomentList(offset, size) {
    const statement = `SELECT 
      ...
      (SELECT JSON_ARRAYAGG(CONCAT('http://121.41.115.226:8000/moment/images/', file.filename)) FROM file WHERE moment.id = file.moment_id) image
